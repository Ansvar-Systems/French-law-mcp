/**
 * search_eu_implementations — Search EU directives/regulations with French implementation info.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_french_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS french_statute_count,
  ...
  if (input.has_french_implementation !== undefined) {
    sql += input.has_french_implementation ? ` HAVING french_statute_count > 0` : ` HAVING french_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    french_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        french_statute_count: r.french_statute_count,
